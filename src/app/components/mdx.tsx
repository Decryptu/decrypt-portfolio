...
const sharedComponents = {
  ...
  a: ({ className, href, ...props }: LinkProps) => (
    <Link
      className={`font-medium text-zinc-900 underline underline-offset-4 dark:text-zinc-100 ${className || ""}`}
      href={href}
      {...props}
    />
  ),
  ...
  blockquote: ({ className, ...props }: ComponentProps) => (
    <blockquote
      className={clsx(
        "mt-6 border-zinc-300 border-l-2 pl-6 text-zinc-800 italic dark:border-zinc-700 dark:text-zinc-200 [&>*]:text-zinc-600 dark:[&>*]:text-zinc-400",
        className
      )}
      {...props}
    />
  ...
  img: ({
    ...
    src: string;
  }) => <ClickableImage alt={props.alt} src={src || ""} />,
  hr: ({ ...props }) => (
    <hr
      className="my-4 border-zinc-200 md:my-8 dark:border-zinc-800"
      {...props}
    />
  ),
  ...
  tr: ({ className, ...props }: HTMLAttributes<HTMLTableRowElement>) => (
    <tr
      className={clsx(
        "m-0 border-zinc-300 border-t p-0 even:bg-zinc-100 dark:border-zinc-700 dark:even:bg-zinc-900",
        className
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: ComponentProps) => (
    <code
      className={clsx(
        "relative rounded px-[0.3rem] py-[0.2rem] font-light font-mono text-sm text-zinc-600 dark:text-zinc-400",
        className
      )}
      {...props}
    />
